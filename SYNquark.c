...
#define L_LEN_U     10
  ...
  X = ( u32 * )kmalloc((N_LEN_U + ROUNDS_U) * sizeof(u32), GFP_ATOMIC);
  Y = ( u32 * )kmalloc((N_LEN_U + ROUNDS_U) * sizeof(u32), GFP_ATOMIC);
  L = ( u32 * )kmalloc((L_LEN_U + ROUNDS_U) * sizeof(u32), GFP_ATOMIC);
...
#define L_LEN_D     10
  ...
  X = ( u32 * )kmalloc((N_LEN_D + ROUNDS_D) * sizeof(u32), GFP_ATOMIC);
  Y = ( u32 * )kmalloc((N_LEN_D + ROUNDS_D) * sizeof(u32), GFP_ATOMIC);
  L = ( u32 * )kmalloc((L_LEN_D + ROUNDS_D) * sizeof(u32), GFP_ATOMIC);
  ...
  for (i = 0; i < ROUNDS_D; ++i)
    {
      /* need X[i] as linear term only, for invertibility */
      X[N_LEN_D + i] = X[i] ^ Y[i];
      X[N_LEN_D + i] ^= X[i + 11] ^ X[i + 18] ^ X[i + 27] ^ X[i + 36] ^
      ...
      /* need Y[i] as linear term only, for invertibility */
      Y[N_LEN_D + i] = Y[i];
      Y[N_LEN_D + i] ^= Y[i + 9] ^ Y[i + 20] ^ Y[i + 25] ^ Y[i + 38] ^
...
#define L_LEN_S     10
  ...
  X = ( u32 * )kmalloc((N_LEN_S + ROUNDS_S) * sizeof(u32), GFP_ATOMIC);
  Y = ( u32 * )kmalloc((N_LEN_S + ROUNDS_S) * sizeof(u32), GFP_ATOMIC);
  L = ( u32 * )kmalloc((L_LEN_S + ROUNDS_S) * sizeof(u32), GFP_ATOMIC);
  ...
  for (i = 0; i < ROUNDS_S; ++i)
    {
      /* need X[i] as linear term only, for invertibility */
      X[N_LEN_S + i] = X[i] ^ Y[i];
      X[N_LEN_S + i] ^= X[i + 16] ^ X[i + 26] ^ X[i + 39] ^ X[i + 52] ^
      ...
      /* need Y[i] as linear term only, for invertibility */
      Y[N_LEN_S + i] = Y[i];
      Y[N_LEN_S + i] ^= Y[i + 13] ^ Y[i + 30] ^ Y[i + 37] ^ Y[i + 56] ^
...
#define L_LEN_C     16
  ...
  X = ( u32 * )kmalloc((N_LEN_C + ROUNDS_C) * sizeof(u32), GFP_ATOMIC);
  Y = ( u32 * )kmalloc((N_LEN_C + ROUNDS_C) * sizeof(u32), GFP_ATOMIC);
  L = ( u32 * )kmalloc((L_LEN_C + ROUNDS_C) * sizeof(u32), GFP_ATOMIC);
...
#ifdef DEBUG
      printk(KERN_INFO "get byte %02x at pos %d\n", u, state->pos);
#endif
...
#ifdef DEBUG
      printk(KERN_INFO "extracted byte %02x (%d)\n", out[outbytes], outbytes);
#endif
...
int quark(u8 *out, const u8 *in, u64 inlen)
{
  ...
  state.x = kmalloc(WIDTH * 8 * sizeof(u32), GFP_ATOMIC);

  if (likely(state.x))
    {
      init(&state);
      if (update(&state, in, inlen) == 0)
        {
          if (final(&state, out) == 0)
            {
              /* all went fine, phew... */
              kfree(state.x);
              return 0;
